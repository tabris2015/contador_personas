...
while True:
	...
	if totalFrames % args["skip_frames"] == 0:
		...
		for (objectID, centroid) in ct.objects.items():
			...
			for cent in to.centroids:
				...
				if sector:
					sector_points[sector][1] += 1
					if sector_points[sector][1] > TIME_LIMIT:
						sector_points[sector][0] += 2
						sector_points[sector][1] = 0
						for sec, points in sector_points.items():
							points[0] -= 1
	...
	for sector, (start, end) in sectors.items():
		...
		cv2.putText(frame, str(sector_points[sector][0]), (start[0], start[1] + 15),
			cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
